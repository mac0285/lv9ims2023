...
<div class="min-h-screen items-center">
        ...
        <form method="get">         
            <input
            type="text"
            class="form-input border-solid border border-gray-300 p-2 w-full md:w-1/2 leading-tight hover:bg-blue-400 hover:text-gray px-6 focus:inline-yes shadow-md rounded-full"
    ...
    <div wire:loading>Processing data...</div>
    <div wire:loading.remove></div>
        ...
        @if ($query == "")
            <div class="text-gray-500 text-sm">
                Enter a term to search for Supplies.
            </div>
            @else
            @if($searchs->isEmpty())
                <div class="text-gray-500 text-sm">No matching result was found.
                </div>
            @else
            <table class="table-fixed bg-with-border text-white w-full md:w-1/2 bg-gray-500 divide-y px-4 py-4 my-3 shadow-md rounded">
                <thead>
                    <tr>
                       <th class="px-6 py-3 text-left text-xs font-medium text-white-500   uppercase tracking-wider">SKU</th>
                     
                        <th class="px-6 py-3 text-left text-xs font-medium text-white-500   uppercase tracking-wider">barcode</th>
                      
                        <th class="px-6 py-3 text-left text-xs font-medium text-white-500   uppercase tracking-wider">item</th>
                        ...
                        <td class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> 
                            <div class="text-xs  font-medium text-gray-900"> 
                                <p class="text-gray-500">
                                    <button wire:click="addtocart({{ $search->id }})"
                                    class=" bg-green-500 hover:bg-green-700 text-white px-3 rounded-full">add to cart:{{$search->sku}} </button></p> </div>
                        </td>
    ...
    <div class="px-4 mt-4"> Masukan sales di sini    </div>
</div></div>
